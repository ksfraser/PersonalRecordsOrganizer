...
class EPM_Admin_Contact_Emails {
    public static function render_page() {
        ...
        $order_by = '';
        // Only order by client_id if the column exists
        $columns = $wpdb->get_col("DESC $table_name", 0);
        if (in_array('client_id', $columns)) {
            $order_by = 'ORDER BY client_id, id';
        } else {
            $order_by = 'ORDER BY id';
        }
        $sql = "SELECT * FROM $table_name $where_sql $order_by";
        ...
        <div class="wrap">
            ...
            <table class="widefat">
                <thead>
                    <tr><th>ID</th><th>Client ID</th><th>Email</th><th>Is Primary</th><th>Created</th><th>Last Updated</th><th>Matching Users</th><th>Invite</th></tr>
                </thead>
                <tbody>
                <?php foreach ($emails as $row): ?>
                    <tr>
                        ...
                        <td><?php
                            $users = function_exists('get_users') ? get_users(['search' => '*' . $row->email . '*', 'search_columns' => ['user_email']]) : [];
                            if ($users) {
